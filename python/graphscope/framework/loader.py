...
class CSVOptions(object):
    ...
    def to_dict(self) -> Dict:
        options = {"delimiter": self.delimiter, "header_row": self.header_row}
...
class Loader(object):
    ...
    def process_location(self, source):
        self.protocol = urlparse(source).scheme or "file"
        self.source = source
    ...
    def process_numpy(self, source: Sequence[np.ndarray]):
        ...
        col_names = [f"f{i}" for i in range(self.column_num)]
    ...
    def process_vy_object(self, source):
        ...
        if isinstance(source, vineyard.Object):
            self.source = f"o{repr(source.id)}"
        elif isinstance(source, vineyard.ObjectID):
            self.source = f"o{repr(source)}"
        elif isinstance(source, vineyard.ObjectName):
            self.source = f"s{str(source)}"
        else:
            raise ValueError(
    ...
    def get_attr(self):
        ...
        if self.protocol == "file":
            source = f"{self.source}#{self.options}"
